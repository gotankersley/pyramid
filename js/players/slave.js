...
//Start - Don't need no stink' post events
...
function init(rootId, horizonSearch) {
	...
	if (!horizonSearch) {
		...
		if (Math.abs(bestScore) == INFINITY) { //End found
			postMessage({tid:tid, score:bestScore});
		...
		var score = Math.abs(bestScore) == VERY_MANY? bestScore + savedBestScore : savedBestScore;
		
		//Send results to master
		postMessage({tid:tid, score:score});
		setTimeout(function() {close(); }, 50);
		return;
	}
}
...
function negascout (player, opp, turn, alpha, beta, depth) { 				
	...
	for (var k = 1; k < playerKids.length; k+=2) { 
		...
		//Win
		if (BB_isWin(kid, turn, destPos)) return VERY_MANY;		
	}
